...
const OUTPUT_DIR = path.join(__dirname, 'data', 'motu');
if (!fs.existsSync(OUTPUT_DIR)) fs.mkdirSync(OUTPUT_DIR, { recursive: true });

async function main() {
  const browser = await puppeteer.launch({ headless: false, defaultViewport: { width: 1280, height: 900 } });
  const page = await browser.newPage();
  await page.goto('https://www.transformerland.com/wiki/masters-of-the-universe-motu/the-original-series/', { waitUntil: 'networkidle2', timeout: 60000 });
  ...
  console.log(`Found ${data.toyLinks.length} MOTU toy links`);
  await browser.close();
}
