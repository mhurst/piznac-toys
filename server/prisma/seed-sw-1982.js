...
async function main() {
  ...
  async function seedList(items, subSeriesId, label) {
    ...
    for (const name of items) {
      const existing = await prisma.figure.findFirst({
        where: { name, toyLineId: toyline.id, seriesId: vintage.id, year: 1984 },
      });
  ...
  await seedList(figures1982, figSubSeries?.id, 'figures');
  await seedList(vehicles1982, vehSubSeries?.id, 'vehicles');
}
