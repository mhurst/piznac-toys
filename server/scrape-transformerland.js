...
const G1_WIKI_URL = `${BASE_URL}/wiki/transformers/g1/`;
...
async function getAllToyUrls(browser) {
  ...
  console.log('Fetching G1 toy listing page...');
  const page = await browser.newPage();
  await page.goto(G1_WIKI_URL, { waitUntil: 'networkidle2', timeout: 60000 });

  const toyUrls = await page.evaluate(() => {
    const links = [];
    document.querySelectorAll('a[href*="/wiki/toy-info/transformers-g1"]').forEach((a) => {
...
async function scrapeToyPage(browser, url) {
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      // Parse title: "Action Masters Grimlock with Anti-Tank Cannon (Transformers, G1, Autobot)"
      const title = document.title || '';
      const titleMatch = title.match(/^(.+?)\s+(.+?)\s*\(Transformers,\s*G1,\s*(\w+)\)/);
      if (titleMatch) {
